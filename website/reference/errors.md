...
| `BOOK_PROFILE_NOT_LINKED`     | The book is not linked to the requested profile. | Saving economics or creating a campaign for a profile the book's ASIN is not mapped to. | Link the ASIN first (`POST /api/books/mappings`). |
