...
describe("MCP tool surface", () => {
  ...
  it("rejects invalid write tool arguments before touching the service", async () => {
    ...
    // Non-numeric recommendation IDs
    const badRecIds = await writeClient.callTool({
      name: "create_recommendation_change_set",
      arguments: { recommendationIds: ["not-a-number"] },
    });
    expect(badRecIds.isError).toBe(true);

    const badRejectId = await writeClient.callTool({
      name: "reject_recommendation",
      arguments: { recommendationId: "not-numeric" },
    });
    expect(badRejectId.isError).toBe(true);
  });

  it("defaults keyword matchType to EXACT when omitted", async () => {
    const fakeWrite = {
      createRecommendationChangeSet: vi.fn(),
      createCampaignNegativesChangeSet: vi.fn(),
      createSearchTermExclusion: vi.fn(),
    ...
    await writeClient.callTool({
      ...
      arguments: {
        campaignId: "camp-1",
        keywords: [{ keywordText: "dragon books" }],
      },
    });
    ...
    expect(fakeWrite.addKeywordsToCampaign).toHaveBeenCalledWith(
      WORKSPACE,
      "camp-1",
      [{ keywordText: "dragon books", matchType: "EXACT" }],
      undefined,
    );
  });
