...
describe("write/negatives", () => {
  describe("createCampaignNegativesChangeSet", () => {
    ...
    it("throws when all terms lack historical clicks and are not ASINs", async () => {
      ...
      await expect(
        createCampaignNegativesChangeSet(pool, "ws-1", "camp-1", [
          "random zero click query",
        ]),
      ).rejects.toThrow("No terms have historical clicks or ASIN targets");
    });

    it("skips zero-click terms when mixed with eligible terms and notes them in metadata", async () => {
      const { pool, tables } = setupMockPool({
        searchTermMetricsDaily: [
          {
            profile_id: "prof-1",
      ...
      await createCampaignNegativesChangeSet(pool, "ws-1", "camp-1", [
        "sci-fi books",
        "random speculative term",
      ]);

      expect(tables.changeSets).toHaveLength(1);
      expect(tables.changeSets[0].metadata.termsWithHistoricalClicks).toEqual([
        "sci-fi books",
      ]);
      expect(tables.changeSets[0].metadata.skippedZeroClickTerms).toEqual([
        "random speculative term",
      ]);
    });
