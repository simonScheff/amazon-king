...
export async function resolveCampaign(
  ...
  const res = await db.query<ResolvedCampaign>(
    `select c.id, c.profile_id, c.amazon_campaign_id, c.name, c.state, c.targeting_type, c.raw_json, p.write_enabled
     from campaigns c
     ...
     where conn.workspace_id = $1 and (c.id::text = $2 or c.amazon_campaign_id = $2)
       and conn.status <> 'disconnected'
     order by case when conn.status = 'connected' then 0 else 1 end, c.id desc
     limit 1`,
...
/**
 * Resolves a campaign and throws if not found, if archived, or if the profile is read-only.
 */
export async function requireCampaign(
  ...
  if (campaign.state.toLowerCase() === "archived") {
    throw new Error(
      `Campaign '${campaign.name}' is archived on Amazon and cannot be modified`,
    );
  }
  if (!campaign.write_enabled) {
    throw new Error(
...
/** Resolves an ad group within a campaign (by ID or falls back to the first non-archived). */
export async function resolveAdGroupId(
  ...
  const res = await db.query<{ id: string }>(
    `select id from ad_groups
     where campaign_id = $1 and state <> 'ARCHIVED'
     order by case when state = 'ENABLED' then 0 else 1 end, id asc
     limit 1`,
    [campaignId],
  );
