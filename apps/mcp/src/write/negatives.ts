...
export async function createCampaignNegativesChangeSet(
  ...
  // campaign_id in search_term_metrics_daily is the Amazon external ID;
  // profile_id filter ensures cross-profile isolation even if an Amazon ID
  // is reused across profiles.
  ...
  const skippedZeroClickTerms = terms.filter(
    (t) => !isAsin(t) && !termsWithClicks.has(t.toLowerCase()),
  );

  if (eligibleTerms.length === 0) {
    throw new Error(
      `No terms have historical clicks or ASIN targets on campaign '${campaign.name}'. Per repository rules, negative keywords must target queries with historical clicks.`,
    );
  }

  const specs: changes.ChangeActionInsert[] = eligibleTerms.map((term) => ({
  ...
  const created = await createFingerprintedChangeSet(pool, {
    ...
    metadata: {
      ...
      termCount: eligibleTerms.length,
      termsWithHistoricalClicks: eligibleTerms,
      skippedZeroClickTerms,
    },
    actions: specs,
    extraFingerprintSeed: [
      { campaignId: campaign.id, searchTerms: eligibleTerms },
    ],
  });
...
export async function createSearchTermExclusion(
  ...
  for (const profile of profileRows) {
    ...
    changeSets.push({
      ...
      profileId:
        (profile as { profile_id?: string; profileId?: string }).profile_id ??
        (profile as { profile_id?: string; profileId?: string }).profileId ??
        profile.id,
