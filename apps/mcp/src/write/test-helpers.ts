  ...
  const pool = {
    async query(sql: string, params: unknown[] = []) {
      ...
      if (
        ...
        const match = tables.campaigns
          .filter((c) => c.id === cId || c.amazon_campaign_id === cId)
          .map((c) => {
            const prof = tables.amazonProfiles.find(
              (p) => p.id === c.profile_id,
            );
            return {
              ...c,
              write_enabled: prof ? (prof.write_enabled ?? true) : true,
            };
          });
        return { rows: match, rowCount: match.length };
      }
      ...
      if (lower.includes("from ad_groups where campaign_id = $1")) {
        ...
        if (lower.includes("state <> 'archived'")) {
          match = match.filter((ag) => ag.state !== "ARCHIVED");
        }
        if (params[1]) {
      ...
      if (lower.includes("from recommendations r")) {
        if (
          lower.includes("r.id = any($2::bigint[])") ||
          lower.includes("r.id::text = any($2::text[])")
        ) {
          const ids = (params[1] as string[]).map(String);
          const match = tables.recommendations
        ...
        if (lower.includes("r.id::text = $2") || lower.includes("r.id = $2")) {
      ...
      if (lower.includes("insert into change_actions")) {
        const lastCs = tables.changeSets[tables.changeSets.length - 1];
        const csId = lastCs ? lastCs.id : String(nextId);
        const rows = [
          {
            id: String(++nextId),
            change_set_id: csId,
