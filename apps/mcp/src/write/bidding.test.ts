...
describe("write/bidding", () => {
  describe("setCampaignMaxCpc", () => {
    it("enforces max CPC only on bids strictly exceeding the ceiling and resets dynamic bidding", async () => {
      ...
      expect(tables.changeSets[0].metadata.campaignId).toBe("camp-1");
      expect(tables.changeSets[0].metadata.maxCpc).toBe("0.50");
    ...
    it("returns null changeSetId when no targets or ad groups exceed the ceiling", async () => {
      const { pool, tables } = setupMockPool({
        targets: [
          {
            id: "tgt-low",
            campaign_id: "camp-1",
            amazon_target_id: "amz-tgt-low",
            target_kind: "target",
            bid: "0.2000",
          },
        ],
      ...
      expect(tables.bidPolicies).toHaveLength(0);
    });

    it("throws when campaign has more than 5,000 bid controls", async () => {
      const manyTargets = Array.from({ length: 5001 }, (_, i) => ({
        id: `tgt-${i}`,
        campaign_id: "camp-1",
        amazon_target_id: `amz-tgt-${i}`,
        target_kind: "target",
        bid: "0.8000",
      }));
      ...
      await expect(
        setCampaignMaxCpc(pool, "ws-1", "camp-1", "0.50"),
      ).rejects.toThrow("more than 5,000 bid controls");
    });

    it("throws when maxCpc is non-positive or invalid", async () => {
      const { pool } = setupMockPool();

      await expect(
        setCampaignMaxCpc(pool, "ws-1", "camp-1", "-0.50"),
      ).rejects.toThrow("Max CPC must be a positive number greater than 0");

      await expect(
        setCampaignMaxCpc(pool, "ws-1", "camp-1", "abc"),
      ).rejects.toThrow("Max CPC must be a positive number greater than 0");
    });
  ...
  describe("setCampaignPlacementMultiplier", () => {
    ...
    it("throws when placement multiplier percentage is invalid (negative, > 900, or non-integer)", async () => {
      const { pool } = setupMockPool();

      await expect(
        setCampaignPlacementMultiplier(pool, "ws-1", "camp-1", {
          topOfSearchPercentage: 901,
        }),
      ).rejects.toThrow(
