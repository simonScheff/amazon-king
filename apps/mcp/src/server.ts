...
function sanitizeError(error: unknown): string {
  ...
  if (
    ...
    raw.includes("Query failed:")
  ) {
    return "Database query error occurred while processing the request.";
  }
  return raw;
}
...
export function buildMcpServer(deps: McpServerDeps): McpServer {
  ...
  if (deps.write) {
    ...
    server.registerTool(
          ...
          recommendationIds: z
            .array(
              z
                .string()
                .trim()
                .regex(/^\d+$/, "Recommendation ID must be a numeric string"),
            )
            .min(1)
            .describe(
      ...
      async ({ recommendationIds }) => {
        try {
          ...
          return notFound(sanitizeError(error));
        }
      },
    );
    ...
    server.registerTool(
      ...
      {
        description:
          "Draft a change set reducing all target and ad-group default bids exceeding maxCpc down to maxCpc. Also resets dynamic bidding to safe legacy bidding (LEGACY_FOR_SALES with 0% placement multipliers) if the campaign currently has active non-legacy bidding strategies or adjustments.",
        inputSchema: z.object({
          campaignId: z
            .string()
            .min(1)
    ...
    server.registerTool(
      ...
      {
        description:
          "Draft a change set to set placement bid multipliers (Top of Search %, Product Pages %) for a campaign.",
        inputSchema: z
          .object({
            campaignId: z
              .string()
      ...
      async ({ campaignId, topOfSearchPercentage, productPagePercentage }) => {
        try {
