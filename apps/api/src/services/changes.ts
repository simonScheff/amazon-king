...
function normalizePlacementItem(
  ...
  const name = String(obj.placement ?? obj.predicate ?? obj.name ?? "").trim();
...
export function createChangeService(deps: ChangeServiceDeps): ChangeService {
  ...
  async function applyLoadedSet(
    ...
    try {
      ...
      const appliedKeywords = finalActions.some(
        (action) =>
          action.status === "applied" && action.actionType === "create_keyword",
      );
      if (
